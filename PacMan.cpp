...
Version: v0.5
Team members (G23_MH):
1. Lee Xin Yi       ; 2103307; MH; [REDACTED]   (Lead developer)
2. Lim Jin Yap      ; 2102460; MH; [REDACTED]   (Developer & Bug hunter)
3. Chua Yun Juan    ; 2103232; MH; [REDACTED]   (Developer & Bug hunter)
4. Joel Lim Yi Hong ; 2102392; MH; [REDACTED]   (Developer & Bug hunter)
5. Ng Zhee Horng    ; 2103167; MH; [REDACTED]   (Developer & Bug hunter)
...
#define SPEEDINMS 50 //Speed of the game in ms
#define CYCLETOSEC 6 //6 cycles in loop = 1 second; Speed is 50ms, theoritically 20 cycles = 1 second; Currently set to 6 as it represent closest to 1 second
...
int main()
{
    ...
    vector<Score> get_scores = { Score(815, 30) };          // Default scores, 815 points in 30 seconds
...
int main_menu()
{
	...
	cout << endl;
    cout << "$$$$$$$\\                                                                  " << endl;  // ASCII art generated by https://patorjk.com/software/taag/
    cout << "$$  __$$\\                                                                 " << endl;
    cout << "$$ |  $$ |$$$$$$\\   $$$$$$$\\ $$$$$$\\$$$$\\   $$$$$$\\  $$$$$$$\\        " << endl;
    cout << "$$$$$$$  |\\____$$\\ $$  _____|$$  _$$  _$$\\  \\____$$\\ $$  __$$\\       " << endl;
    cout << "$$  ____/ $$$$$$$ |$$ /      $$ / $$ / $$ | $$$$$$$ |$$ |  $$ |            " << endl;
    cout << "$$ |     $$  __$$ |$$ |      $$ | $$ | $$ |$$  __$$ |$$ |  $$ |            " << endl;
    cout << "$$ |     \\$$$$$$$ |\\$$$$$$$\\ $$ | $$ | $$ |\\$$$$$$$ |$$ |  $$ |        " << endl;
    cout << "\\__|      \\_______| \\_______|\\__| \\__| \\__| \\_______|\\__|  \\__|   " << endl;
    cout << endl;
    ...
    cout << endl;
    cout << "Current level: " << loaded_level(0, 0) << endl;  // Display current level
    cout << "Press a/d to change level (" << files_id << " out of " << txt_files.size() - 1 << ") "<< endl;  // Display total number of levels
...
void runGame(int* pScore, int* pGlobalTime)
{
    ...
    for (int i = 0; i < COLS + 1; ++i)  // initialize invinsible ghost walls
    {
    ...
    for (int count = 0; true; count++) // infinite loop, with count running
    {
        ...
        if (count % 2 == 0)  // ghost move every 2 rounds
        {
        ...
        CONSOLE_CURSOR_INFO cursorInfo;                 // Next 4 lines set cursor visibility to FALSE, so terminal will not flash
        GetConsoleCursorInfo(consoleHandle, &cursorInfo);
        cursorInfo.bVisible = FALSE;
        SetConsoleCursorInfo(consoleHandle, &cursorInfo);

        for (int row = 0; row < ROWS *2; row++) // draw 80*40 screen
        {
            for (int column = 0; column < COLS *2; column++)
            {
                ...
                else if (game_wall_coords[row/2][column/2] == 1)  // Print walls
                {
					cout << '#';
                }
        ...
        cout << "Time taken: " << setw(4) << timetaken++ / CYCLETOSEC << "s" << endl;  // Speed is 50ms, theoritically 20 cycles = 1 second; Currently set to 6 as it represent closest to 1 second
        cout << "Press esc to quit" << endl;

        Sleep(SPEEDINMS);  // Speed control

        if (isDotsLeft == false) // Check if there is any dots left, win if no dots left
        {
            cout << endl;
            cout << "You win!" << endl;
            cout << "Score: " << score << endl;
            cout << "Time taken: " << setw(4) << timetaken / CYCLETOSEC << "s" << endl;
            cout << "Press any key to continue" << endl;
            
            globaltime = timetaken/CYCLETOSEC;
            
            _getch();
            break;
        }
        else if ((x == g_x || x == g_x + 1 || x == g_x - 1) && (y == g_y || y == g_y + 1 || y == g_y - 1)) // Check if player position is same / next to ghost position, lose if true
        {
            cout << endl;
            cout << "You lose!" << endl;
            cout << "Score: " << score << endl;
...
void level_editor()
{
	...
	int x = 0, y = 0; 	//record editor cursor's position
	int wall_coords [20][40] = { 0 }; //record wall coordinates, 0 = no wall, 1 = wall

	ifstream infile;
	infile.open(loaded_level(0, 0));  //Read txt file
	if (!infile)
		infile.close(); //file error
    ...
    {
        wall_coords[yy][xx] = 1;  //Set respective wall coordinates to 1
    }
	...
	for (;;)
	{
		...
		if (_kbhit())
		{
			...
			{
				ofstream outfile;
				outfile.open(loaded_level(0, 0));  //Write to txt file
				if (!outfile)
				...
				for (int r = 0; r < 20; r++)
				{
					for (int c = 0; c < 40; c++)
					{
						if (wall_coords[r][c] == 1)
							outfile << c << " " << r << endl;  //Write wall coordinates to txt file
					}
				}
                break;
			}
...
void level_create()
{
    ...
    ofstream outfile;
    outfile.open(custom_level_name + ".txt");  //Create txt file
    if (!outfile)
        outfile.close(); //file error
